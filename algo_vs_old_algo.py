...
def main():
    ...
    game = Game(SIZE, Player("better"), Player("random"))
    ...
    input_shape = (19, 19)
    agents = [Agent(input_shape,input_shape,nr_actions, learning_rate=0.001, discount_factor=0.99, load_weights = True), Agent(input_shape,input_shape,nr_actions, learning_rate=0.001, discount_factor=0.99, load_weights = False)]
    #(self, state_shape1, state_shape2, num_actions,canvas, trainer, actions, learning_rate=0.001, discount_factor=0.99) 
    trainer = Trainer(input_shape,agents, game, "better", "random")
    if(VISUALIZE):
        x_position += -100
        y_position += -100
        # Set the position of the display window
    ...
    trainer.agents[1].set_training_file("random.h5")
    ...
    while iterations != 0:            # connect(game,node1, node2, canvas)
        ...
        if trainer.game.check_game_finished():       
            # time.sleep(0.1)
            trainer.step()
            print(trainer.last_score)
            if(trainer.last_score == []):
                continue
            print("\nMore trained got this diff: ")
            cur_diff = trainer.last_score[0] - trainer.last_score[1]
            print(cur_diff)
            print("\noverall diff: ")
            overall_diff = overall_diff + cur_diff
            print(overall_diff)
            print("\naverage diff:")
            print(overall_diff/iteration_nr)
            iterations += -1
